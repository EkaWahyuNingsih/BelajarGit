...
private DefaultTableModel model; //untuk membuat model pada data
    ...
    private boolean kebenaran=true; //mendeklarasikan variabel kebenaran  yang akan bersangkutan dengan pemanggilan antara validasi dan tambah data
...
private void HapusData(String id, int baris){//Menghapus data yang tersimpan pada database
    try{
            ...
            model.removeRow(baris);//untuk menghapus baris pada tabel
        }catch(SQLException e){
            System.out.println(e.getMessage());//Perintah untuk menampilkan Pesan Error jika eksekusi Gagal atau ada yang salah.
        }
    }
private void CariId(){
   ...
   private void CariJudul(){
    ...
    private void ValidasiData(String judul, String penulis, String harga){
        try{
            String sql = "select*from buku";//menampilkan data yang ada pada tabel buku
            stt = con.createStatement();//membuat dan menghubungkan statment
            rss = stt.executeQuery(sql);//Mendefinisikan untuk mendapatkan data dengan eksekusi Query
            while(rss.next()){//selama masih ada data (next)
                Object[] o = new Object[2];//Menambahkan baris data object Tabel
                o[0] = rss.getString("judul").toLowerCase();//berfungsi untuk mmenyeleksi judul ketika ingin menambahkan data tetapi judulnya sama dengan data yang telah ada maka validassinya tidak akan tersmpan data yang telahh dibuat
                o[1] = rss.getString("penulis").toLowerCase();//berfungsi untuk mmenyeleksi penulis ketika ingin menambahkan data tetapi penulisnya sama dengan data yang telah ada maka validassinya tidak akan tersmpan data yang telah dibuat

                
                if(o[0].equals(judul.toLowerCase()) && o[1].equals(penulis.toLowerCase())){//penyeleksian untuk validasi yang telah dibuat pada script diatas
                   
                    JOptionPane.showMessageDialog(null,"Data Sudah Ada");///menampilan pesan ketika data yang ingin ditambahkan sudah ada
                    kebenaran=false;//jika sama maa program akan berhenti
                    break;
            }
            }
            if(kebenaran==true){//jika benar maa program akan mengeksekus data an menyimpannya 
...
// TODO add your handling code here:
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
      ...
      String judul = jTextField1.getText();//Kolom Judul sebagai String
        ...
        ValidasiData(judul,penulis,harga);// Meng-inisialisasikan Validasi data dan memanggilnnya pada button simpan
        InitTable(); //Meng-inisialisasi Table
        TampilData();//Meng-inisialisasi Tampilan data 
